...
def onboard_images_charts():
    cmd="sh " + scriptHome + "ncm_onboarding_script.sh " +str(ncm_admin_user)+" "+str(ncm_admin_pwd)+" "+str(control_ip)+" "+str(bcmt_port)+" "+str(sdl_package_path)
    os.system(cmd)
...
def labInstall():
    ...
    cmd="sh " + scriptHome + "bringUpSdl.sh " +str(sdl_ns)+" "+str(pgw_ns)+" "+str(sdl_install_path)+" "+str(pgw_install_path)+" "+str(sdl_crd_path)+" "+str(pgw_crd_path)+" "+"sdl"
    os.system(cmd)
...
def labUnInstall():
    cmd="sh " + scriptHome + "deleteSdl.sh " +str(sdl_ns)+" "+str(pgw_ns)+" "+str(sdl_install_path)+" "+str(pgw_install_path)+" "+str(sdl_crd_path)+" "+str(pgw_crd_path)+" "+"sdl"
    os.system(cmd)

def provisionSS_sdl():
    lcmCommon.logger.info("============provisionSS_sdl========")
    cmd = pythonPath + " ./provisionSS_sdl.py "
    msg = "provisionSS_sdl failed with error: "
    lcmCommon.cmd_exec(cmd, msg)
...
class sdlInstall(lcmCommon.installCnfProduct):
